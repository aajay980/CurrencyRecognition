...
for f in $1/twohundred/*.jpg; do echo $f;done > train_files/twohundred.txt
for f in $1/fivehundred/*.jpg; do echo $f;done > train_files/fivehundred.txt
for f in $1/new_hundred/*.jpg; do echo $f;done > train_files/new_hundred.txt
for f in $1/new_ten/*.jpg; do echo $f;done > train_files/new_ten.txt


g++ `pkg-config --cflags --libs opencv` currency.cpp -o currency-train -lopencv_core -lopencv_imgproc -lopencv_highgui -lopencv_objdetect -lopencv_xfeatures2d -lopencv_features2d -lopencv_flann -lopencv_imgcodecs;
